...
def edit_photo(record_id):
    try:
        ...
        # record_id에 맞는 레코드 검색
        c.execute("SELECT * FROM photo_records WHERE id = ?", (record_id,))
        record = c.fetchone()

        if record is None:
            return "사진을 찾을 수 없습니다.", 404  # 사진이 없으면 404 오류 페이지를 반환하거나 다른 처리를 할 수 있습니다.
        ...
        if request.method == 'POST':
            ...
            if photo and allowed_file(photo.filename):
                ...
                # 기존 파일 경로 확인 (디버깅용 출력 추가)
                old_file_path = os.path.join(app.config['UPLOAD_FOLDER'], str(record[1]))
                print(f"기존 파일 경로: {old_file_path}")

                # 파일 삭제
                if os.path.exists(old_file_path):
                    os.remove(old_file_path)  # 파일이 존재하면 삭제
                else:
                    print("기존 파일이 존재하지 않습니다.")  # 파일이 존재하지 않으면 출력
        ...
        # filename을 str로 변환하여 템플릿에 전달
        return render_template('edit_photo.html', filename=str(record[1]), record_id=record_id)

    except sqlite3.Error as e:
        # DB 연결이나 실행 오류 처리
        return f"DB 오류 발생: {str(e)}", 500
